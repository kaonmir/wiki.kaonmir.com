...
6. **Execution Engine**의 구성 요소인,
    ...
    - [[#Garbage Collector|GC]]는 **JVM**이 적절할 때 실행하여 Heap 영역의 미참조 객체를 정리합니다.
...
### Method Area
[[C언어|C언어의 코드 영역]]에 대응하는, 메타데이터와 바이트 코드(명령어)를 저장하는 데이터 영역입니다. (당연하게도) Method Area의 모든 정보는 클래스별로 관리됩니다.
...
[[C언어|C언어의 Heap 영역]]과 비슷하게 객체 인스턴스와 배열을 저장하는 데이터 영역입니다. `new` 연산자로 생성된 객체와 배열의 실제 데이터는 모두 여기에 저장합니다. Heap에 생성된 객체와 배열의 주소는 스택에 저장해 사용합니다.
...
### 참조
- [Permanent Generation 영역은 힙일까 아닐까?](https://blog.naver.com/simjunbo/221080829920)
