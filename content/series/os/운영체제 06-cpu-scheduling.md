...
[스케줄러 액티베이션](./운영체제%2004-thread#스케줄러-액티베이션) 참조하면, 스레드 라이브러리는 사용자 수준 스레드를 사용 가능한 LWP에 스케줄링한다. 그리고 운영체제는 커널 수준 스레드를 CPU에 스케줄링한다.
...
[멀티 프로세서](./운영체제%2001-intro#멀티-프로세서) 에서 배운 것처럼 NUMA에서 CPU는 가까운 메인 메모리에 빠르게 접근할 수 있다. 그렇기 때문에 운영체제의 알고리즘을 적절히 사용한다면, 할당하고 싶은 CPU에 제일 가까운 위치에 프로세스를 싣을 수도 있다.
...
[실시간 임베디드 시스템](./운영체제%2001-intro#실시간-임베디드-시스템) 에서 실시간 임베디드 시스템이 사용하는 스케줄링이다. 여기서는 스케줄되는걸 서비스를 받는다고 표현하는 것 같다.
...
[운영체제 연산](./운영체제%2001-intro#운영체제-연산) 에서 현대 운영체제는 인터럽트 구동식이라고 했었다. 이 방식은 `인터럽트(사건) → 디스패치 → 응답`의 과정을 거친다. **지연 시간**은 사건 발생부터 응답까지 걸린 시간이다.
