...
[공유 메모리 시스템](./운영체제%2003-process#공유-메모리-시스템) 에서 언급했던 것처럼 버퍼에 여러 프로세스가 동시에 값을 넣는다면 어떻게 되는지 알아본다. 간단하게 변수에 1을 더하고 값을 가져와보자. 단 2개의 프로세스가 동시에 작업을 수행한다.
...
[OpenMP](./운영체제%2004-thread#openmp) 파트에서 설명한 OpenMP는 `#pragma omp parallel`로 병렬 처리를 지원하고, `#pragma omp cirtical`로 치명적인 구역에서 하나의 스레드만 실행할 수 있게 한다. 사용하기는 매우 쉽지만, OpenMP는 mutex처럼 동작하기 때문에 교착상태가 발생할 수 있다.
